...
export default function EditorialCarDetailPage() {
    ...
    async function fetchCar() {
      try {
        ...
        if (!snap.exists()) {
          ...
          const data = { id: snap.id, ...snap.data() } as Car;
          setCar(data);
          setMessage(
            `Hola, estoy viendo la nota editorial sobre el ${data.title} (${data.price}). ¿Sigue disponible?`
          );
        }
      } catch (err) {
  ...
  async function handleLeadSubmit(e: FormEvent) {
    ...
    // Goes through /api/contact (same path the homepage contact form uses)
    // rather than writing to Firestore directly from the client — that
    // keeps the lead in the exact shape lib/db/leads.ts and the dashboard's
    // Consultas table expect (vehicleId, stage, preferredContact, …), and
    // avoids needing a public Firestore write rule on `leads` just for
    // this page.
    try {
      ...
      if (!res.ok || !data.ok) throw new Error(data.error ?? "No se pudo enviar la consulta.");
      setSuccess(true);
    } catch (err) {
